...
- **Chip "💾 Ocupará ~N GB"** bajo el nombre del MKV. No es una predicción: el pipeline **copia** los flujos sin recodificar, así que es contabilidad — vídeo (por diferencia) más los `StreamSize` de las pistas de audio que sobreviven, por el factor del contenedor. `estimate_output_size_bytes` (en `phase_b`) lo calcula y `GET /api/sessions/{id}` lo sirve como `estimated_size_bytes`, **campo calculado y NO persistido**: cambia en cuanto el usuario toca la selección de pistas.
  ...
  - **La base es `playlist_size`, no `FileSize`.** Un m2ts puede contener más de lo que su playlist reproduce y lo que acaba en el MKV es lo segundo; mkvmerge lo da en `container.properties.playlist_size`. Medido sobre el corpus del NAS, **16 de 44 discos difieren más de un 2 %**: Minions & Monsters tiene un m2ts de 91,4 GB del que el playlist usa 70,0 (el MKV salió de 64,0 — con `FileSize` la cuenta daba 81 GB, un +26,6 %), y los episodios de Juego de Tronos tienen m2ts de 33-98 GB con playlists de ~32. También cubre el caso inverso, el playlist **mayor** que el m2ts, que es el seamless branching de Avatar Fuego y Ceniza.
  - **El tamaño del audio sale de `StreamSize`, y si falta, de `BitRate × duración`.** `StreamSize` no está en 18 de las 44 sesiones y ahí no se estimaba nada; sobre las 250 pistas donde MediaInfo da los dos, el error mediano del bitrate es **+0,0 %** (p10 −0,4 %). Hay un 22 % de pistas donde los dos números se separan una barbaridad, pero son **todas del mismo disco** y ahí el que miente es el `StreamSize`: 1-2 MB por pista TrueHD en una película de 1h44. Se prefiere `StreamSize` igualmente porque es una medida y no una extrapolación.
  - **Devuelve `None` —y el chip se oculta— en cuanto el dato no es fiable**, en vez de enseñar un cero o un "?". Tres guards: `TOPE_MBPS_FISICO = 128` (por encima del pico físico de un BD100 UHD la base no puede ser del título; con `playlist_size` ya casi no dispara, queda como red), `TOPE_AUDIO_SOBRE_BASE = 0.25` (el audio es el 5,4 % del fichero en la mediana y el 13,7 % en el peor caso medido, así que por encima de eso el dato describe un error de lectura, no un disco) y **una pista sin `StreamSize` ni `BitRate` lo cancela todo**, porque el vídeo sale por diferencia y sin todos los tamaños la resta miente — es lo que oculta Avatar Fuego y Ceniza, y es correcto. Una cifra inventada con pinta de dato es peor que un hueco.
  - Medido sobre las 49 sesiones del NAS: el chip sale en **35** (antes 21) y el peor error contra el fichero real es del **6,8 %** (antes 9,8 %). `fixtures_estimacion/golden_estimacion_tamano.json` recorta 10 discos reales y `test_estimacion_tamano.py` comprueba el valor exacto **y** que el error contra el MKV real cabe en el 10 %. Va en su propio directorio a propósito: `fixtures_discos/` lo recorre entero `test_golden_discos_reales` con un `glob` y un JSON con otra forma le rompe el `setUpClass`.
...
- **Sin tope de proyectos abiertos** en sub-tabs (`MAX_PROJECTS = Infinity` en `core.js`). El tope de 5 se quitó cuando llegó el modo serie: un disco de una temporada abre un proyecto por episodio y chocaba con el límite. Tab 3 sí mantiene su 5 (`MAX_CMV40_PROJECTS`) — un job CMv4.0 es mucho más pesado y no tiene caso multi-episodio.
...
- **Punto verde animado** en cada tab principal (`tab-running-dot-{1,2,3}`) cuando ese tab tiene jobs activos. Polling cada 5s: `queueState` en memoria para Tab 1 (que cuenta también lo *encolado*, y `activity` solo conoce lo que corre) y **una sola** petición a `/api/activity` para los otros dos. Visible aunque el usuario esté en otro tab, pero **el poller se salta las vueltas con `document.hidden`** — era tráfico cada 5 s durante horas contra un NAS que está procesando vídeo; al volver, `visibilitychange` dispara `_runRecoveryTasks`, que los refresca.
...
- **Fase D contrasta offsets, y el de la hoja NO es un desfase pendiente**: es el que la comunidad **detectó y ya corrigió** dentro del bin, así que lo que se espera medir es **cero**. `sheet_sync_hint()` (en `phases/cmv40_pipeline.py`, junto a `detect_sync_offset`) devuelve `corregido` (|residuo| ≤ 2 frames → la corrección está puesta) y `parece_sin_corregir` (el residuo coincide en magnitud con lo documentado, en cualquiera de los dos signos → este bin no parece el corregido). Se expone en `GET /api/cmv40/{id}/sync-data` como `sheet_sync`. **Estuvo al revés**: premiaba que el residuo coincidiera con lo documentado, con el texto «confirmación fuerte de que el bin está bien alineado». Sobre el corpus del NAS eso marcaba «algo no cuadra» en los **16 proyectos con offset documentado ≠ 0, ninguno de los cuales necesitó corregir nada** — entre ellos La trama fenicia con −584 frames (24 s) documentados, que completó por drop-in sin tocar el sync. Confirmado midiendo Minions & Monsters: hoja +16, residuo 0 con Pearson 1,0 y RMS 0,0.
...
### El criterio de calidad: la MAGNITUD del L8, y solo eso

Recalibrado el 2026-09-18 sobre **40 bins del repo DoviTools** (18 retail,
22 generados) más **dos pares controlados** —mismo título con las dos
procedencias, que es el experimento limpio porque el título deja de ser una
variable.

**La ciencia, de la documentación de Dolby:**

| nivel | quién lo crea | ¿automático? | ¿en el BD? |
|---|---|---|---|
| L1 brillo por frame | Dolby Analysis | sí | sí |
| **L3** mid-tone offset per-shot | **Dolby Analysis** | **sí** | no |
| L2 trims CMv2.9 | cm_analyze + colorista | casi | **sí** |
| **L8** trims CMv4.0 | **solo colorista** | **no** | no |

Literal: *«L8 is not generated and must fully come from the colorist»* y
*«Mid Tone Offset is generated by the Dolby Analysis»*. Por eso **L8 es el
único nivel que puede decidir**: un L8 pegado al neutro es lo que
`cm_analyze` produciría sobre tu propio disco.

**Y lo que decide es la MAGNITUD, no el conteo:**

```
maxΔ = mayor desviación de slope/offset/power/chroma/saturation/
       mid_contrast/clip_trim respecto a 2048     (ms_weight NO: su neutro es 0)

real     si maxΔ > 50      (L8_REAL_MINIMAL_SIGNIFICANT_DELTA, ya calibrado)
default  si no
```
...
entre ellos Posesión infernal, con 2 combos y `power −184`, `sat −328` en
158.942 frames. El % de frames neutros tampoco: hay retail al 81 % y
generados al 33 %.

**Tres cosas que costaron el estudio y conviene no volver a descubrir:**

- **`ms_weight` vale 0, no 2048.** Contarlo en la comprobación de
  neutralidad daba un combo enteramente neutro por «trabajado», y dejaba
  `l8_neutral_pct` a 0 % en bins neutros de arriba abajo.
- **La magnitud se DERIVA de los combos si el campo no viene**
  (`delta_l8_de`). Un `RpuAnalysis` se construye por cuatro vías y si el
  criterio dependiera solo del campo, tres de ellas darían 0 y **todo
...
veredicto hasta que se relance el análisis extendido — **no se bumpea
`CACHE_VERSION_QUALITY`**, que serían ~10 min de `extract-rpu` por MKV.

**Dónde se ve cada nivel.** La card de Tab 3 dejó de ser una lista de
números sueltos («combos L8: 2», que no dice si eso es bueno) y pasa a una
tabla de cuatro columnas —nivel · qué aporta · el bin · **tu disco**— con el
L8 marcado como el que decide. Esa última columna es la clave didáctica:
enseña de un vistazo que L1 y L2 ya los tienes y que lo único que el bin
añade de un humano es el L8. En Tab 2, el stat de L8 de la radiografía lleva
el `maxΔ` al lado del conteo.
...
**L3 NO entra en el veredicto, y está medido.** Hubo un «rescate por L3»
durante unas horas de ese mismo día y **era un bug**: sobre 40 bins el
conteo de combos L3 acierta el **57 %** —azar— y su mediana es MAYOR en los
generados (1.256) que en los retail (1.003), así que habría rescatado
exactamente los bins que hay que descartar. El motivo es el de arriba: el
mid tone offset lo produce el análisis de Dolby, no el colorista.
...
medidos, **ninguno** traía L3 identidad.
