...
async function openFileBrowser({ title, subtitle, roots, onSelect } = {}) {
  ...
  if (listEl) listEl.innerHTML = '<div class="file-browser-loading"><span data-icono="reloj"></span> ' + tr('ui.cargando') + '</div>';
...
async function fileBrowserNavigate(relPath) {
  ...
  try {
    ...
    if (listEl) listEl.innerHTML = `<div class="file-browser-empty"><span data-icono="aviso"></span> ${tr('comun.error_p1', {p1: escHtml(e.message || String(e))})}</div>`;
  }
}
